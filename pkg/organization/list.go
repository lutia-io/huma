...
func buildListQuery(params listParams) (countSQL, listSQL string, countArgs, listArgs []any) {
	...
	if params.OrganizationID != "" {
		where = append(where, "o.id = "+b.add(params.OrganizationID))
	}
	if params.Query != "" {
		...
		where = append(where, fmt.Sprintf(
			"(o.name ILIKE %s ESCAPE '%s' OR o.slug ILIKE %s ESCAPE '%s' OR n.name ILIKE %s ESCAPE '%s')",
	...
	listSQL = "SELECT" + organizationListSelectColumns + fromSQL + fmt.Sprintf(" ORDER BY %s %s, o.id ASC", orderColumn, order)
