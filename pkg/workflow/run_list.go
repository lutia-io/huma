...
const durationExpr = `EXTRACT(EPOCH FROM (COALESCE(w.completed_at, NOW()) - w.created_at))`

var runListSortColumns = map[string]string{
	"name":          "COALESCE(wd.name, '')",
	"status":        "w.status",
...
func parseRunListParams(r *http.Request) (runListParams, error) {
	...
	if params.Status != "" {
		if _, ok := workflowStatuses[params.Status]; !ok {
...
func buildRunListQuery(params runListParams) (countSQL, listSQL string, countArgs, listArgs []any) {
	...
	if params.OrganizationID != "" {
		where = append(where, "w.organization_id = "+b.add(params.OrganizationID))
	}
	if params.Query != "" {
		...
		where = append(where, fmt.Sprintf(
			"(wd.name ILIKE %s ESCAPE '%s' OR w.status ILIKE %s ESCAPE '%s' OR o.name ILIKE %s ESCAPE '%s' OR n.name ILIKE %s ESCAPE '%s')",
	...
	listSQL = "SELECT" + workflowSelectColumns + fromSQL + fmt.Sprintf(" ORDER BY %s %s, w.id ASC", orderColumn, order)
